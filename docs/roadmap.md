...
Internal development plan. Order is by priority, not strict
sequencing -- v0.4 might land before v0.3 if a need overtakes the
queue.
...
* 204 tests; verified end-to-end laptop -> host_d with vibe-qc importable
...
### v0.5.3 — CRYSTAL14 + PROPERTIES14 on host_d (2026-05-09)
...
* `web_state` test fixture isolates `ENV_CONFIG_DIR` +
  ...
  when the suite ran on host_d for the first time).
...
answers the host_d/host_a operator's "is my job actually using the cores? how
...
* **REMOTE-2** — uses `_ssh_base`: `ConnectTimeout` / `BatchMode=yes` /
  ...
  instead of hanging the fetch (the 2026-05-26 host_d-lockout class).
...
branch drift loudly (the 2026-05-25 host_d vibeqc-dev on
...
* ``--yes`` is required. The reset is destructive (any
  ...
  edits after a cross-chat collision on host_d / host_a), but
...
The 2026-05-26 host_d ssh-trust lockout exposed an architectural
...
The 2026-05-26 host_d ssh-trust lockout exposed a long-latent gap:
...
fleet-update incident: host_d vibeqc-dev silently checked out
...
3. **`--update-script-arg FLAG`** (repeatable). Forwards
   ...
   three times on host_d; this verb closes it.
...
* **v0.7.5 *Hopper's Compiler*** — shipped 2026-05-26. 3-tier
  ...
  script. Motivated by the 2026-05-26 host_d lockout. See
...
The "don't write to the git repo on host_d/host_a" ship. Provoked
by the 2026-05-25 deployment incident where untracked dirs on
host_d (108 files in `examples/periodic/`,
`examples/experimental_regression/`) and modified basis files on
host_a (141 `.g94` citation-comment additions, 1 deleted `.ecp`)
...
* New [`vibe-queue/docs/agent_interaction.md`](agent_interaction.md):
  protocol for dev chats running on host_d / host_a (TL;DR,
...
* **v0.6.56**: persist `update_script_output` in `vq admin
  ...
  host_a's dirty-tree error today).
...
**Fleet status:** host_d + host_a both have a token file
...
Fleet impact: host_d + host_a both run multi-user and need this —
...
**Fixes a token-rotation footgun hit live on host_a.** Rotating the
host_a admin token with `sudo /opt/vq/venv/bin/vq web init-token
...
scrollback. During the host_a bring-up that output was pasted into a
...
script (host_d, host_a) were surfaced and should be rotated —
...
never dispatched. The host_d bring-up hit exactly this.
...
appended to every user's personal config. (host_d and host_a
...
updating host_a after it came back online: `vq summary host_a` showed
...
them — running **N + max_jobs** jobs. On host_a: 2 orphans (from the
...
host_d, this audited whether the watchdog still protects jobs that
...
live on host_d, the first real submit surfaced a hole: per-user
...
**Verified live:** multi-user is deployed on host_d — a test job
...
update vibeqc-release on host_d now?" or "did this overnight
...
host_d wedge revealed an OOM cascade that took out vq-daemon
...
clean no-op. The fleet's actual production hosts (host_d +
host_a, both Linux) get the protection.
...
**Polish on v0.6.12.** End-to-end smoke test on host_a caught it:
...
`vq admin update vibeqc-queue host_d host_a`.
...
host_d host_a` picks up the new flag via the editable install
path; the daemon's behavior doesn't change.

### v0.6.9 — `vq resubmit` cleans inherited daemon artifacts (2026-05-18)

**Polish fix on v0.6.8.** End-to-end smoke test on host_d after
...
**Remote-host path**: `vq resubmit host_d JOBID` forwards over
...
host_d + host_a picks up the new verb via the editable
...
* **`docs/hosts.md`** — added `crystal23demo` and
  ...
  tables for both host_d and host_a; added them to the
...
**Fleet smoke from v0.6.3** validated end-to-end: host_d
...
**Fleet capability addition.** Both host_a and host_d have
...
reconfigures anything. Would have caught today's 2026-05-17 host_d
...
**Today's host_d incident, replayed through this verb:**
...
* `TestVerifyContractOrphanSystemdUser` (1) — pgrep returns a PID
  but systemctl can't reach it (today's host_d symptom).
...
codebase audit run after the 2026-05-17 host_d incident.** Not
...
code. On host_d with 30+ release-branch jobs running, this
...
v0.5.42 deploy attempt to host_a (the chat that wrote v0.5.42 handed off
before deploying; the deploy chat caught the bug on the first
``vq admin update vibeqc-queue host_a`` call — no banner, no PID
...
system interpreter (``/usr/bin/python3.14`` on host_a / host_d).
...
**Closes the stale-code gap surfaced by 2026-05-16's host_d
...
vibeqc-queue`, but the host_d daemon kept running v0.5.39 in
...
host_d (commit `285e9a9`) but the user reported the box still went
"quite unresponsive and then it dies" during a re-run later the same
day. Forensics:

* v0.5.40 budgeted **10 GB / worker** with no hard cap. On host_d
...
|---------|------:|--------:|----:|--------:|--------------:|
| host_d |    32 | 125 GB  |  6  | ~60 GB  | ~65 GB        |
| host_a    |    16 |  62 GB  |  4  | ~40 GB  | ~22 GB        |
...
* ``test_safe_build_parallelism_formula`` updated for the new
  ...
  minimum, hard cap on a 256 GB machine, plus the host_d baseline.
...
**Durability fix.** Direct follow-up to the host_d global-OOM hang
...
ninja parallelism is `nproc` — 32 on host_d — and each cc1plus on
...
| Host    | nproc | RAM     | cap | peak     | host headroom |
|---------|------:|--------:|----:|---------:|--------------:|
| host_d |    32 | 125 GB  |  12 | ~120 GB  | ~5 GB         |
| host_a    |    16 |  62 GB  |   6 |  ~60 GB  | ~2 GB         |
...
fields in both host_d's and host_a's ``~/.config/vq/config.toml``
...
--all` made it obvious that host_a's vibeqc-dev was 7 commits behind
host_d's. Refreshing both via one command is the natural follow-on.
...
Predicted-and-shipped item: once host_a came online as a second compute
host (2026-05-15), typing `vq queue host_d; vq queue host_a` for routine
...
```json
{
  "text":    "vq: job mgo-pbe-abc123def456 finished as completed (rc=0) on host_d",
  "content": "<same>",
  "job": {
    "id":            "abc123def456",
    "name":          "mgo-pbe",
...
**Diagnosis verified live**: host_d had a stray `/home/USER/output.out`
...
laptop: `bash -c 'echo MARKER > /tmp/marker.txt'` landed on host_d,
...
-e .` on the laptop picks it up; on host_d, pulling + reinstalling
...
host_d is a shared workstation (interactive workloads, occasional reboots) now
...
* **Note on host_d specifically**: auto-resume fires when the daemon
  *next starts*. host_d's daemon currently runs via ``nohup`` (the
  ...
  the daemon auto-starts is the orthogonal host_d-infra follow-up
...
* **Remote case** ``vq tail host_d JOBID -f --name vibeqc.log``
  delegates as ``ssh host_d vq tail localhost JOBID -f --name
...
chat-debug question: "is host_d at the commit I just pushed?"
...
* **Delegation**: ``vq admin status`` on the laptop SSH-forwards to
  ``ssh host_d vq admin status localhost``; the remote vq's git
...
* **Delegation**: ``vq admin update vibeqc-release --tag v0.8.0``
  ...
  ``ssh host_d vq admin update vibeqc-release localhost --tag v0.8.0``;
...
archive directory was hard-coded to ``<state_root>/archive/``; host_d
...
v0.5.13 roadmap entry but punted because host_d was always
cgroup-enforced. That assumption broke in this session: systemd-user
on host_d hung, daemon now runs via nohup with ``cgroup=disabled``.
The fallback is suddenly the only path to throttle on host_d.
...
tag: a chat that pushes a commit then submits a job needs host_d to
have that commit. Before v0.5.20 the chat had to either ask the operator to
``ssh host_d && git pull && bash scripts/update-dev.sh`` manually, or
...
* **Delegation pattern** matches every other vq verb: ``vq admin
  update vibeqc-dev`` on the laptop SSH-forwards to ``ssh host_d vq
  admin update vibeqc-dev localhost``. The registry lives where the
  binaries do (host_d); the laptop just dispatches the request.
...
* **``contrib/run-crystal.sh``** honors four env-var binary overrides:
  ...
  (system /usr/bin/mpirun on host_d; non-issue).
...
SSH) only knew about live workspaces. Confirmed live on host_d
...
v0.5.6 smoke testing on host_d (jobs `dd4fef514889`,
...
won't fire in that mode. host_d uses cgroup mode (kernel
...
`systemctl --user set-property`. Live smoke-test on host_d
...
* host_d config wires up both vibe-qc clones: `--branch main`
...
* Manual venv-refresh procedure documented (`vq pause --all` →
  ssh host_d → `git pull && bash scripts/update.sh` → `vq resume
...
* **Multi-user backbone** — shipped in v0.6.x (May 2026).
  ...
  target host (host_d confirmed willing).
...
`remote_vq --version` on the scheduler host, so host_f onboarding can verify the
...
walltime, so host_f accounting avoids charging qsub queue wait as compute time.
...
so host_f batches can use array metadata and chain dependencies without depending
...
Issue #136 exposed a stricter trust-boundary gap on host_d: the root-owned
...
supported host_d deployment model and repairing exact legacy workspaces still
